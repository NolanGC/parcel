...
// Browser build. The host already has a DOM — a native one — so jsdom is not
// merely unnecessary here, it is the single largest dependency we get to drop.
//
// `DOMParser.parseFromString` yields an inert document: scripts do not run and
// `src` attributes are not fetched, so nothing in the message loads or executes
// during conversion. That inertness is the whole reason this is safe to do on
// the client.
